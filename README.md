...
This is `terraform-aws-route53-resolver-dns-firewall` project provides all the scaffolding for a typical well-built Cloud Posse module. It's a template repository you can
...
This module is used to configure the [Route 53 Resolver DNS Firewall](https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/resolver-dns-firewall.html).
It is setup in an allow list mode and blocks all domains not in the allow list.

* Note that when using subdomains all associated CNAME records must also be added to the allow list, you can use the `recurse_cnames`
  flag to auto generate this list and add it to the allowed domains if wanted. This uses a python script as an [external data source](https://registry.terraform.io/providers/hashicorp/external/latest/docs/data-sources/data_source)
  and the [dnspython](https://www.dnspython.org/) library to get all CNAME records.
